...
قائمة جانبية عربية كاملة + شروحات + واجهة احترافية
"""
...
.main .block-container{{
  padding:0 clamp(12px,2.5vw,22px) 60px!important;
...
[data-testid="stSidebar"]{{
  ...
  border-left:1px solid var(--sbrd)!important;
...
.sb-section-title{{
  font-size:0.75rem;font-weight:700;color:var(--bl);
  letter-spacing:1.5px;text-transform:uppercase;
  margin-bottom:12px;
  display:flex;align-items:center;gap:6px;
}}
.sb-hint{{
  background:rgba(59,130,246,{"0.12" if DARK else "0.07"});
  border-right:3px solid var(--bl);
  border-radius:0 8px 8px 0;
  padding:8px 12px;margin-top:8px;
  font-size:0.75rem;color:var(--txt2);line-height:1.6;
}}
.sb-warn{{
  background:rgba(245,158,11,{"0.12" if DARK else "0.07"});
...
.sb-logo{{
  text-align:center;padding:20px 10px 14px;
}}
.sb-logo-name{{
  font-size:1.15rem;font-weight:800;color:var(--bl);
...
.sb-divider{{
  height:1px;background:var(--brd);margin:12px 0;
}}
...
div[data-baseweb="select"]>div{{
  ...
  font-size:0.88rem!important;color:var(--txt)!important;min-height:42px!important;
}}
...
div[data-baseweb="input"]>div{{
  background:var(--inp)!important;border:1px solid var(--inpb)!important;
  border-radius:10px!important;font-family:var(--font)!important;
  color:var(--txt)!important;min-height:42px!important;
}}
...
div[data-testid="stRadio"] label{{
  background:var(--card2)!important;border:1px solid var(--brd)!important;
  border-radius:10px!important;padding:8px 14px!important;
  font-size:0.85rem!important;font-weight:500!important;
  color:var(--txt)!important;transition:all .2s!important;
}}
div[data-testid="stRadio"] label:has(input:checked){{
  background:rgba(59,130,246,{"0.2" if DARK else "0.1"})!important;
  border-color:var(--bl)!important;color:var(--bl)!important;
}}
div[data-testid="stProgressBar"]>div>div{{
  background:linear-gradient(90deg,#1D4ED8,#4F46E5)!important;border-radius:4px!important;
}}
div[data-testid="stProgressBar"]>div{{background:var(--brd)!important;border-radius:4px!important;}}

/* Buttons */
.stButton>button{{
  font-family:var(--font)!important;font-size:0.92rem!important;
  font-weight:700!important;border-radius:12px!important;
  padding:11px 22px!important;transition:all .2s!important;
  border:none!important;min-height:44px!important;width:100%!important;
}}
...
.habi-hdr{{
  ...
  margin:0 clamp(-12px,-2.5vw,-22px) 0;
...
.hdr-name{{font-size:clamp(1.1rem,3.5vw,1.45rem);font-weight:800;color:#fff;line-height:1.1;}}
.hdr-sub{{font-size:0.65rem;color:rgba(255,255,255,0.55);
...
/* Market Clock */
.mkt{{background:rgba(0,0,0,0.28);border:1px solid rgba(255,255,255,0.12);
      border-radius:12px;padding:8px 13px;display:flex;align-items:center;gap:10px;}}
...
.tick-wrap{{background:{"#060810" if DARK else "#1E1B4B"};overflow:hidden;height:32px;
            display:flex;align-items:center;margin:0 clamp(-12px,-2.5vw,-22px);}}
...
.ti{{display:inline-flex;align-items:center;gap:6px;padding:0 20px;
     font-size:0.75rem;font-weight:500;color:rgba(255,255,255,0.82);
     border-left:1px solid rgba(255,255,255,0.08);}}
.ts{{color:#93C5FD;font-weight:700;}} .tu{{color:#4ADE80;}} .td{{color:#F87171;}}

/* ─── SEC HEADER ─── */
.sh{{display:flex;align-items:center;gap:10px;margin:20px 0 12px;}}
.sh-ico{{width:34px;height:34px;background:rgba(59,130,246,{"0.2" if DARK else "0.1"});
...
/* ─── KPI GRID 3x2 ─── */
.kgrid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));
        gap:clamp(8px,1.5vw,13px);margin-bottom:4px;}}
@media(max-width:600px){{.kgrid{{grid-template-columns:repeat(2,minmax(0,1fr));}}}}
.kc{{background:var(--card);border-radius:14px;
...
.kc::after{{content:'';position:absolute;right:0;top:0;
...
.ki{{width:clamp(40px,7vw,48px);height:clamp(40px,7vw,48px);border-radius:10px;
     display:flex;align-items:center;justify-content:center;
     font-size:clamp(1.1rem,2.5vw,1.3rem);flex-shrink:0;}}
...
/* ─── META BANNER ─── */
.meta{{background:var(--card);border:1px solid var(--brd);border-radius:14px;
...
/* ─── DRILL BADGE ─── */
.drillb{{background:{"linear-gradient(135deg,#0F1D4A,#1A1060)" if DARK else "linear-gradient(135deg,#EFF6FF,#F0EDFF)"};
         border:1px solid {"#1E3A8A" if DARK else "#C7D2FE"};border-radius:14px;
         padding:11px 17px;display:flex;align-items:center;justify-content:space-between;
...
@media(max-width:768px){{.habi-hdr{{flex-direction:column;align-items:flex-start;}}
  .hdr-right{{width:100%;justify-content:flex-start;}}.mkt{{width:100%;}}}}
...
# ══════════════════════════════════════════════════════
def _ghex(g):
    return {"A+":GR,"A":BL,"B":AM,"C":RD}.get(g,TXT3)
...
def _is_exp(ts,days=3): return _age(ts)>=days*24*60
...
def sh(icon,ar,en=""):
    ...
    st.markdown(f'<div class="sh"><div class="sh-ico">{icon}</div>'
                f'<div><div class="sh-ar">{ar}</div>{e}</div>'
                f'<div class="sh-ln"></div></div>',unsafe_allow_html=True)

def kpi(icon,num,ar,en,cls):
    return (f'<div class="kc {cls}"><div class="ki {cls}">{icon}</div>'
            f'<div class="kb"><div class="kn {cls}">{num}</div>'
            f'<div class="kar">{ar}</div><div class="ken">{en}</div></div></div>')

def footer():
    st.markdown(
        f'<div class="ft"><div class="ftar">© جميع الحقوق محفوظة للحبي</div>'
        ...
        f'quantomoption.com</a></div></div>',unsafe_allow_html=True)
...
def _ticker(radar_df):
    if radar_df is not None and not radar_df.empty:
        ...
        for _,row in top.iterrows():
            sym=str(row.get("Ticker","?")); bias=str(row.get("Bias","—"))
            entry=row.get("Entry","—"); rr=row.get("Best R:R","—")
            liq=row.get("نوع السيولة","—")
            bc="tu" if bias=="Long" else "td"
            bt="شراء ▲" if bias=="Long" else "بيع ▼"
            items.append(f'<span class="ti"><span class="ts">{sym}</span>'
        ...
        items=[
            ...
            '<span class="ti"><span class="ts">SPY</span><span style="color:#93C5FD">انتظار MSS</span></span>',
        ]
    inner="".join(items*2)
    st.markdown(f'<div class="tick-wrap"><div class="tick-inner">{inner}</div></div>',
                unsafe_allow_html=True)
...
<div class="habi-hdr">
  ...
  <div class="hdr-right">
    <div class="mkt">
      ...
      <div><div class="mkt-st">{status}</div>
           <div class="mkt-cd">{cd_lbl}: <b>{cd}</b></div></div>
      <div class="mkt-sep"></div>
      <div><div class="mkt-t">{time_s}</div>
           <div class="mkt-d">{date_s}</div></div>
    </div>
    {fresh}
...
# ══════════════════════════════════════════════════════
#  SIDEBAR — القائمة الجانبية العربية الكاملة
# ══════════════════════════════════════════════════════
def render_sidebar():
    with st.sidebar:
        ...
        <div class="sb-logo">
          ...
          <div class="sb-logo-name">منصة الحبي للتداول</div>
          <div class="sb-logo-sub">Habbi Trading Platform</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # ── وضع الإضاءة ──
        lbl = "☀️ تبديل للوضع النهاري" if DARK else "🌙 تبديل للوضع الليلي"
        ...
        st.markdown('<div class="sb-section-title">📊 نوع التحليل</div>', unsafe_allow_html=True)

        mode_choice = st.radio(
            "اختر النوع",
            ["📈 تحليل سهم واحد", "📡 مسح الرادار (جميع الأسهم)"],
        ...
        st.markdown("""
        <div class="sb-hint">
        📈 <b>تحليل سهم واحد:</b> تحليل عميق لسهم محدد مع شارت كامل وخطة صفقة<br><br>
        📡 <b>مسح الرادار:</b> فحص عشرات الأسهم دفعة واحدة وتصنيفها حسب جودة الإعداد
        </div>
        """, unsafe_allow_html=True)
        ...
        # ══════════════════════════════════════════════
        st.markdown('<div class="sb-section">', unsafe_allow_html=True)
        st.markdown('<div class="sb-section-title">🔍 الرمز والزوج المرجعي</div>', unsafe_allow_html=True)

        ticker = st.text_input(
            "رمز السهم",
            value=st.session_state.get("single_ticker","TSLA"),
            placeholder="مثال: TSLA",
            disabled=(mode=="radar")
        ).strip().upper()

        smt = st.text_input(
            "زوج SMT المرجعي",
            value=st.session_state.get("single_smt","QQQ"),
            placeholder="مثال: QQQ أو SPY",
            disabled=(mode=="radar")
        ).strip().upper()

        st.markdown("""
        <div class="sb-hint">
        🔤 <b>رمز السهم:</b> أدخل رمز السهم باللغة الإنجليزية — مثل TSLA أو AAPL أو NVDA<br><br>
        🔗 <b>زوج SMT:</b> سهم مرجعي للمقارنة. عادةً QQQ للتقنية أو SPY للسوق العام.<br>
        الهدف: الكشف عن الانحراف بين السهمين (تلاعب مؤسسي)
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

        # ══════════════════════════════════════════════
        #  قسم 3 — قائمة المراقبة (للرادار)
        # ══════════════════════════════════════════════
        st.markdown('<div class="sb-section">', unsafe_allow_html=True)
        st.markdown('<div class="sb-section-title">📋 قائمة المراقبة</div>', unsafe_allow_html=True)

        WATCHLIST_AR = {
            "جميع الأسهم — 70 سهم":         "Options (70 Stocks)",
            "أسهم التقنية الكبرى — 30 سهم":  "Tech Mega-Cap (30)",
            "الأسهم القيادية S&P — 40 سهم":  "Blue Chips S&P (40)",
            "الأسهم الرخيصة — أقل من $20":   "Cheap Stocks (<$20)",
            "صناديق الكريبتو":                 "Crypto ETFs",
        }
        preset_ar = st.selectbox(
            "اختر القائمة",
            list(WATCHLIST_AR.keys()),
            disabled=(mode=="single"),
            label_visibility="collapsed"
        )
        preset    = WATCHLIST_AR[preset_ar]
        watchlist = E.WATCHLIST_PRESETS[preset]

        st.markdown(f"""
        <div class="sb-hint">
        القائمة المختارة تحتوي على <b>{len(watchlist)} سهم</b><br>
        سيتم فحص جميع الأسهم بالتوازي وترتيبها حسب جودة الإعداد (A+ أولاً)
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

        # ══════════════════════════════════════════════
        #  قسم 4 — إعدادات الشارت
        # ══════════════════════════════════════════════
        st.markdown('<div class="sb-section">', unsafe_allow_html=True)
        st.markdown('<div class="sb-section-title">📈 إعدادات الشارت</div>', unsafe_allow_html=True)

        HTF_AR = {
            "يومي (1D) — الأفضل للاستراتيجية":  "1d",
            "أسبوعي (1W) — نظرة بعيدة":          "1wk",
            "4 ساعات (4H) — تأكيد إضافي":        "4h",
            "ساعة (1H) — تفاصيل الدخول":         "1h",
        }
        htf_ar = st.selectbox("الإطار الزمني الرئيسي", list(HTF_AR.keys()))
        htf_tf = HTF_AR[htf_ar]

        CANDLES_AR = {
            "40 شمعة — عرض سريع":    40,
            "60 شمعة — متوازن":       60,
            "80 شمعة — مفصّل (مُوصى)": 80,
            "100 شمعة — تاريخ أوسع":  100,
            "150 شمعة — أقصى تفصيل":  150,
        }
        candles_ar = st.selectbox("عدد الشموع في الشارت", list(CANDLES_AR.keys()), index=2)
        n_can = CANDLES_AR[candles_ar]

        st.markdown("""
        <div class="sb-hint">
        📅 <b>الإطار اليومي (1D)</b> هو الأنسب لاستراتيجية الحبي لأن السحب يكون على مستويات يومية<br><br>
        🕯️ <b>عدد الشموع:</b> كلما زاد العدد، ظهر تاريخ أوسع لكن الشارت يصبح أبطأ قليلاً
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

        # ══════════════════════════════════════════════
        #  قسم 5 — إعدادات المحرك
        # ══════════════════════════════════════════════
        st.markdown('<div class="sb-section">', unsafe_allow_html=True)
        st.markdown('<div class="sb-section-title">⚙️ إعدادات المحرك المتقدمة</div>', unsafe_allow_html=True)

        SCORE_AR = {
            "4 نقاط — يُظهر أكثر الإعدادات":   4,
            "5 نقاط — متوازن (مُوصى)":           5,
            "7 نقاط — إعدادات قوية فقط":         7,
            "10 نقاط — إعدادات ذهبية فقط":       10,
        }
        score_ar = st.selectbox("الحد الأدنى للنقاط", list(SCORE_AR.keys()), index=1)
        min_sc = SCORE_AR[score_ar]

        WICK_AR = {
            "5%  — يقبل أي سحب":          5,
            "8%  — موصى (الافتراضي)":     8,
            "15% — سحب متوسط القوة":      15,
            "25% — سحب قوي فقط":          25,
            "40% — سحب قوي جداً":         40,
        }
        wick_ar = st.selectbox("قوة السحب الأدنى (Wick)", list(WICK_AR.keys()), index=1)
        min_wk = WICK_AR[wick_ar]

        GRADE_AR = {
            "A+ فقط — أفضل إعداد":           ["A+"],
            "A+ و A — إعدادات ممتازة (مُوصى)": ["A+","A"],
            "A+ و A و B — جميع الإعدادات":    ["A+","A","B"],
            "الكل بدون استثناء":               ["A+","A","B","C"],
        }
        grade_ar = st.selectbox("تصفية حسب جودة الإعداد", list(GRADE_AR.keys()), index=1)
        gf = GRADE_AR[grade_ar]

        st.markdown("""
        <div class="sb-hint">
        🏆 <b>النقاط:</b> المحرك يحسب نقاط من 0–13 لكل إعداد<br>
        &nbsp;&nbsp;&nbsp;• <b>A+ (11+)</b>: إعداد ذهبي نادر<br>
        &nbsp;&nbsp;&nbsp;• <b>A (8-10)</b>: إعداد قوي جداً<br>
        &nbsp;&nbsp;&nbsp;• <b>B (5-7)</b>: إعداد جيد<br><br>
        📉 <b>قوة السحب:</b> نسبة ذيل الشمعة التي تمثل السحب. كلما زادت النسبة كان السحب أقوى وأكثر موثوقية
        </div>
        """, unsafe_allow_html=True)

        st.markdown("""
        <div class="sb-warn">
        ⚠️ <b>تذكير:</b> هذه المنصة للأغراض التعليمية فقط وليست نصيحة مالية. التداول ينطوي على مخاطر.
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        run_btn  = st.button("▶ ابدأ التحليل", type="primary",
                              use_container_width=True,
                              disabled=(mode=="radar"))
        scan_btn = st.button("📡 ابدأ مسح الرادار",
                              use_container_width=True,
                              disabled=(mode=="single"))
...
def kpi_single(setup, df_d, dol):
    ...
    chg=(cur-prev)/prev*100; sign="+" if chg>=0 else ""; cc="gr" if chg>=0 else "rd"
    ...
    gc={"A+":"gr","A":"bl","B":"am"}.get(setup.grade if setup else "","tl")
...
def kpi_radar(df):
    n_ap=len(df[df["Grade"]=="A+"]); n_a=len(df[df["Grade"]=="A"])
    n_b=len(df[df["Grade"]=="B"]); n_lng=len(df[df["Bias"]=="Long"])
    ...
    st.markdown(f'<div class="sbar">{seg(ap_w,GR)}{seg(a_w,BL)}{seg(b_w,AM)}{seg(sk_w,BRD)}</div>',
                unsafe_allow_html=True)
...
def render_dlog(setup,cur):
    if not setup:
        st.markdown('<div class="dlog"><div style="padding:26px;text-align:center;">'
                    ...
                    '</div></div>',unsafe_allow_html=True); return
...
def render_trade_table(setup,ticker,ts):
    ...
    for t in setup.targets:
        ...
        rows.append({"cls":cls,"L":t.label,"P":f"{t.price:.4f}",
                     "D":f"{abs(t.price-setup.entry):.4f}","RR":f"1:{rr:.1f}","N":f"level {t.level:.0f}"})
...
def _style_radar(show):
    ...
    return (show.style
            ...
            .set_table_styles([
                ...
                {"selector":"tbody tr:hover td","props":f"background-color:{TBLHOV};"},
            ]))
...
def render_single(ticker,smt,cfg,run_btn):
    ...
    if ts_ex and _is_exp(ts_ex):
        st.warning("⏰ التحليل المحفوظ منتهي الصلاحية (أكثر من 3 أيام). اضغط ابدأ التحليل.")
    if run_btn:
    ...
    if res is None:
        st.markdown('<div class="ph"><span class="phi">📊</span>'
                    '<div class="pht">في انتظار التحليل · Awaiting Analysis</div>'
                    '<div class="phb">اضبط الرمز في القائمة الجانبية<br>'
                    'واضغط <b>▶ ابدأ التحليل</b> لبدء البروتوكول الكامل</div></div>',
                    unsafe_allow_html=True)
        footer(); return
    if _is_exp(st.session_state.single_ts):
        st.markdown('<div class="ph"><span class="phi">⏰</span>'
                    ...
                    'اضغط <b>🔄 تحديث</b> ثم <b>▶ ابدأ التحليل</b></div></div>',
                    unsafe_allow_html=True)
        footer(); return
    ...
    with cc:
        fig=E.build_chart(df_d,setup,liq_lvls,sw_d,dol,
                          ticker=sym,n_candles=cfg["n_can"],htf_interval=cfg["htf_tf"])
...
def render_radar(watchlist,preset,cfg,scan_btn):
    ...
    if ts_r and _is_exp(ts_r):
        st.warning("⏰ نتائج الرادار منتهية الصلاحية. اضغط 🔄 ثم ابدأ مسح الرادار.")
    df=st.session_state.radar_df
    if df is None:
        st.markdown('<div class="ph"><span class="phi">📡</span>'
                    '<div class="pht">الرادار في وضع الاستعداد · Radar Standby</div>'
                    '<div class="phb">اختر قائمة المراقبة من القائمة الجانبية<br>'
                    'واضغط <b>📡 ابدأ مسح الرادار</b></div></div>',
                    unsafe_allow_html=True)
        footer(); return
    if _is_exp(ts_r):
        st.markdown('<div class="ph"><span class="phi">⏰</span>'
    ...
    st.markdown(f'<div class="rtbl"><div class="rth">'
                f'<span class="rtha">🏆 الإعدادات القابلة للتنفيذ · A+ أولاً</span>'
                f'<span class="rthc">{len(show)} من {len(df)}</span>'
                f'</div>',unsafe_allow_html=True)
    st.dataframe(_style_radar(show),use_container_width=True,hide_index=True,height=470)
    st.markdown("</div>",unsafe_allow_html=True)
    st.markdown("<br>",unsafe_allow_html=True)
    sh("🔬","تحليل تفصيلي","Drill Down")
    if show.empty:
        st.info("لا نتائج تطابق الفلاتر الحالية."); footer(); return
...
def _drill(tkr,df,watchlist,cfg):
    ...
    row=df[df["Ticker"]==tkr]; grade=row["Grade"].values[0] if not row.empty else "—"
    gc=_ghex(grade); pill=_gpill(grade); ts_r=st.session_state.radar_ts
    with st.spinner(f"تحميل {tkr}…"):
        try:
            ...
            st.error(f"خطأ: {ex}"); return
    st.markdown(f'<div class="drillb">'
    ...
    with cc:
        fig=E.build_chart(df_d,setup,liq_lvls,sw_d,dol,
                          ticker=tkr,n_candles=cfg["n_can"],htf_interval=cfg["htf_tf"])
        st.plotly_chart(fig,use_container_width=True,
                        config={"displayModeBar":True,"displaylogo":False})
    with lc:
        st.markdown("<br>",unsafe_allow_html=True); render_dlog(setup,cur)
    if setup:
        st.markdown("<br>",unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════
def main():
    mode,ticker,smt,watchlist,preset,cfg,run_btn,scan_btn = render_sidebar()
